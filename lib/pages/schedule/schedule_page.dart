...
class _ScheduleCard extends StatelessWidget {
  const _ScheduleCard({required this.rule, required this.controller, required this.strings});
    ...
    return Card(
      child: ExpansionTile(
        ...
        subtitle: Wrap(
          ...
          children: [
            Chip(
              label: Text(rule.isEnabled ? strings.t('schedule.enabled') : strings.t('schedule.disabled')),
          ...
          ButtonBar(
            ...
            children: [
              TextButton(
                ...
                child: Text(rule.isEnabled ? strings.t('schedule.pause') : strings.t('schedule.resume')),
              ),
              TextButton(
...
class _MaintenanceSection extends StatelessWidget {
  const _MaintenanceSection({required this.controller, required this.strings});
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<List<MaintenanceTask>>(
      ...
      builder: (context, tasks, _) {
        final upcoming = controller.dueSoon(withinDays: 5);
        if (upcoming.isEmpty) {
          return Padding(
            padding: const EdgeInsets.symmetric(vertical: 12),
            child: Text(strings.t('maintenance.empty')),
          );
        }
                  ...
                  subtitle: Column(
                    ...
                    children: [
                      ...
                      Wrap(
                        spacing: 8,
                        children: [
                          _dueChip(task, strings, context),
                          Chip(
                            label: Text(dummyPods.firstWhere((pod) => pod.id == task.podId).name),
                            visualDensity: VisualDensity.compact,
                          ),
                        ],
                      ),
                  ...
                  trailing: Column(
                    ...
                    children: [
                      IconButton(
                        ...
                        onPressed: () => controller.toggleCompleted(task.id),
  ...
  Chip _dueChip(MaintenanceTask task, AppLocalizations strings, BuildContext context) {
    final now = DateTime.now();
    late String label;
    Color color;

    if (task.dueDate.isBefore(now.subtract(const Duration(hours: 1)))) {
      label = strings.t('maintenance.overdue');
      color = Theme.of(context).colorScheme.error;
    } else if (task.dueDate.day == now.day &&
        task.dueDate.month == now.month &&
        task.dueDate.year == now.year) {
      label = strings.t('maintenance.due_today');
      color = Theme.of(context).colorScheme.primary;
    } else {
      final days = task.dueDate.difference(now).inDays + 1;
      label = strings.t('maintenance.due_in').replaceFirst('{days}', days.toString());
