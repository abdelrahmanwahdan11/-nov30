...
class _DashboardPageState extends State<DashboardPage> {
  ...
  late final MaintenanceController maintenanceController = MaintenanceController();
  ...
  Widget _buildMaintenance(AppLocalizations strings) {
    return ValueListenableBuilder<List<MaintenanceTask>>(
      valueListenable: maintenanceController.tasks,
      builder: (context, tasks, _) {
        final upcoming = maintenanceController.dueSoon(withinDays: 4);
        return GlassContainer(
          child: Padding(
            padding: const EdgeInsets.all(16),
            child: Column(
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(strings.t('maintenance.title'), style: Theme.of(context).textTheme.titleMedium),
                          Text(strings.t('maintenance.subtitle'), style: Theme.of(context).textTheme.bodySmall),
                        ],
                      ),
                    ),
                    IconButton(
                      onPressed: maintenanceController.refresh,
                      icon: const Icon(Icons.refresh),
                      tooltip: strings.t('maintenance.refresh'),
                    )
                  ],
                ),
                const SizedBox(height: 8),
                if (upcoming.isEmpty)
                  Text(strings.t('maintenance.empty'))
                else
                  Column(
                    children: upcoming
                        .map(
                          (task) => Padding(
                            ...
                            child: _MaintenanceTile(
                              task: task,
                              strings: strings,
                              onMarkDone: () => maintenanceController.toggleCompleted(task.id),
                              onSnooze: () => maintenanceController.postpone(task.id),
                            ),
                          ),
                        )
                        .toList(),
...
class _MaintenanceTile extends StatelessWidget {
  const _MaintenanceTile({required this.task, required this.strings, required this.onMarkDone, required this.onSnooze});
  ...
  @override
  Widget build(BuildContext context) {
    final label = _maintenanceLabel(task, strings, context);
    final pod = dummyPods.firstWhere((p) => p.id == task.podId);
    return Container(
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
      ...
      child: Row(
        children: [
          CircleAvatar(
            backgroundColor: task.priority.color.withOpacity(0.12),
            child: Icon(Icons.build_rounded, color: task.priority.color),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('${pod.name} • $label', style: Theme.of(context).textTheme.bodySmall),
              ],
            ),
          ),
          IconButton(
            icon: Icon(task.isCompleted ? Icons.check_circle : Icons.check_circle_outline),
            tooltip: strings.t('maintenance.mark_done'),
            onPressed: onMarkDone,
          ),
          IconButton(
            icon: const Icon(Icons.snooze),
            tooltip: strings.t('maintenance.snooze'),
            onPressed: onSnooze,
          ),
        ],
      ),
  ...
  String _maintenanceLabel(MaintenanceTask task, AppLocalizations strings, BuildContext context) {
    final now = DateTime.now();
    if (task.dueDate.isBefore(now.subtract(const Duration(hours: 1)))) {
      return strings.t('maintenance.overdue');
    }
    if (task.dueDate.day == now.day && task.dueDate.month == now.month && task.dueDate.year == now.year) {
      return strings.t('maintenance.due_today');
    }
    final days = task.dueDate.difference(now).inDays + 1;
    return strings.t('maintenance.due_in').replaceFirst('{days}', days.toString());
  }
}
